...
#define INI_PART 3000
#define DIM_BOX 3000
*/
#define DIM_RANDOM 37
#define INI_PART 3
#define DIM_BOX 5
...
void DO_move(Brwre* particle){
	if (particle->ERROR_NUMBER==0)
	{
		...
		printf("Before: %f ", particle->data->First->LS_value_dbl[0]);
		SET_times(particle);

		printf("Middle: %f ", particle->data->First->LS_value_dbl[0]);

		printf("    another time: %f ", particle->data->First->LS_value_dbl[0]);

		printf(" yat : %f ", particle->data->First->LS_value_dbl[0]);

		printf(" yyat: %f ", particle->data->First->LS_value_dbl[0]);

		printf("    After: %f  \n", particle->data->First->LS_value_dbl[0]);
...
void REPEAT_move(Brwre* particle){
	...
	while(particle->ERROR_NUMBER == 0 && particle->END_reached == 0){
		...
		PRINT_list_dbl(particle->data, 0);
		PRINT_list_int(particle->data, 0);
		PRINT_list_int(particle->data, 1);
		printf(" \n Error number: %d and count = %d and END_reached = %d \n", particle->ERROR_NUMBER,
			particle->count, particle->END_reached);
...
int main()
{
	...
	double spatial_noise[5] = {0,0,5,-10,0};
	double times[37] = {1,2,4,2,6,3,5,7,4,9,1,11,3,6,2,4,5,1,7,8,3,6,4,5,1,2,3,4,5,6,7,6,5,4,3,2,1};
	...
	double times_vector[3] = {0,0,0};

	/*List * data, double * vector_dbl, int* vector_int, int dim*/
	INI_list(ini_pos, times_vector, &pos_choice_vector[0][0], 3);

	/*Brwre* particle, double* spat_noise, double* times, List* ini_pos, double aim */
	INI_brwre(particle, spatial_noise, times, ini_pos, 15);
